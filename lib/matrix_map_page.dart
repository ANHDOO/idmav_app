...
class MatrixMapPageState extends State<MatrixMapPage> {
  ...
  // Tùy chọn tìm kiếm - Nguồn dữ liệu (Mặc định offline)
  final SearchSource _searchSource = SearchSource.offline;
  ...
  void _autoDetectProvincesFromKMZ(LatLngBounds bounds, {bool skipFitCamera = false}) {
    ...
    Future.microtask(() async {
      try {
        ...
        // --- [XÓA] FALLBACK: GỌI API NẾU ASSETS KHÔNG CÓ DỮ LIỆU ---
        debugPrint("⚠️ Không tìm thấy trong assets, bỏ qua fallback API theo yêu cầu.");
  ...
  void _executeSearch() {
    // Luôn thực thi tìm kiếm offline
            ...
            content: SizedBox(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    const Text(
                      "Tìm kiếm Tuyến đường (Dữ liệu Offline):",
