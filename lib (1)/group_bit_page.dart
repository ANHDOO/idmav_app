...
class GroupBitPage extends StatefulWidget {
  ...
  @override
  State<GroupBitPage> createState() => _GroupBitPageState();
}

class _GroupBitPageState extends State<GroupBitPage> {
        ...
        return AlertDialog(
          ...
          title: const Row(
            children: [
              ...
              SizedBox(width: 8),
              Expanded(child: Text('AI Đề xuất Nhóm', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold))),
            ],
          ),
          content: finalProposals.isEmpty 
                    ...
                    children: [
                      Container(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          label: Text('TẠO TẤT CẢ (${finalProposals.length} nhóm)', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                          onPressed: () {
                            setState(() {
                              for (var entry in finalProposals) {
                      ...
                      Expanded(
                        child: ListView.builder(
                          shrinkWrap: true,
                          itemCount: finalProposals.length,
                          itemBuilder: (context, index) {
                            final entry = finalProposals[index];
                            return Card(
                              ...
                              child: ListTile(
                                ...
                                subtitle: Text(entry.value.take(3).map((e)=>e.name).join(", ") + "...", maxLines: 1, overflow: TextOverflow.ellipsis),
                                trailing: IconButton(
                        ...
                        content: SizedBox(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(children: [
                                  Expanded(child: InkWell(onTap: () => setStateSearch(() => onlyShowUnused = !onlyShowUnused), child: Row(children: [SizedBox(height: 24, width: 40, child: Switch(value: onlyShowUnused, activeColor: Colors.orange, onChanged: (val) => setStateSearch(() => onlyShowUnused = val))), const SizedBox(width: 4), const Expanded(child: Text('Chưa dùng', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis))]))),
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_groups.isNotEmpty)
            IconButton(icon: const Icon(Icons.delete_sweep, color: Colors.white), tooltip: 'Xóa tất cả', onPressed: _deleteAllGroups),
          IconButton(icon: const Icon(Icons.psychology, color: Colors.white), tooltip: 'AI Đề xuất nhóm', onPressed: _analyzeAndSuggestGroups),
          IconButton(icon: const Icon(Icons.add_circle_outline, size: 28, color: Colors.white), onPressed: () => _showGroupDialog(), tooltip: 'Tạo nhóm mới')
        ],
